...
// ============================================================
import { AdvancedAnalytics, AnalyticsReport, TraceSpan, PerformanceMetrics } from './advanced-analytics.js';
...
export class AnalyticsTools {
  ...
  private async generateReport(options: AnalyticsToolOptions): Promise<AnalyticsReport> {
  ...
  private getTraces(): Array<{ id: string; name: string; duration: number; startTime: Date; children: number }> {
  ...
  private getMetrics(): Array<{ timestamp: string; model: string; responseTime: number; cost: number; success: boolean }> {
  ...
  private getAlerts(): Array<{ id: string; name: string; enabled: boolean; lastTriggered?: string }> {
  ...
  async startTrace(name: string, attributes: { [key: string]: any } = {}): Promise<string> {
  ...
  async addAlert(name: string, condition: any, channels: string[] = []): Promise<string> {
    return this.analytics.addAlert({
      name,
      description: `Alert for ${condition.metric} ${condition.operator} ${condition.threshold}`,
      condition,
      channels: channels.map(channel => ({ type: 'email' as const, target: channel })),
      enabled: true,
      cooldown: 15
    });
  }
...
export const analyticsReportTool: Tool = {
  name: "analytics_report",
  description: "Generate comprehensive analytics reports with performance metrics, trends, and insights for Krab usage optimization.",
  parameters: z.object({
    startDate: z.string().optional().describe("Start date for the report (ISO format)"),
    endDate: z.string().optional().describe("End date for the report (ISO format)"),
    includeTrends: z.boolean().optional().describe("Include time-series trends in the report"),
...
export const analyticsMonitoringTool: Tool = {
  name: "analytics_monitoring",
  description: "Monitor real-time analytics, traces, and performance metrics for observability and debugging.",
  parameters: z.object({
    action: z.enum(["traces", "metrics", "alerts", "export"]).describe("Monitoring action to perform"),
    includeTraces: z.boolean().optional().describe("Include trace data in export"),
    format: z.enum(["json", "csv"]).optional().describe("Export format")
  }),
...
export const analyticsAlertTool: Tool = {
  name: "analytics_alert",
  description: "Manage analytics alerts for monitoring performance, costs, and reliability thresholds.",
  parameters: z.object({
    action: z.enum(["list", "add", "update", "remove"]).describe("Alert management action"),
    alertName: z.string().optional().describe("Name for new alert"),
    alertMetric: z.enum(["responseTime", "errorRate", "cost", "tokenUsage"]).optional().describe("Metric for alert condition"),
    ...
    alertId: z.string().optional().describe("Alert ID for update/remove actions")
  }),
    ...
    try {
      switch (args.action) {
        ...
        case 'remove':
          ...
          const removed = analyticsTools.analytics.removeAlert(args.alertId);
          return {
            success: removed,
            output: removed ? `Alert ${args.alertId} removed successfully` : `Alert ${args.alertId} not found`
          };
...
// ── Tracing Tool ───────────────────────────────────────────
export const analyticsTracingTool: Tool = {
  name: "analytics_tracing",
  description: "Distributed tracing for request flow analysis and performance debugging across Krab components.",
  parameters: z.object({
    action: z.enum(["start", "end", "event", "get"]).describe("Tracing action to perform"),
    ...
    attributes: z.record(z.any()).optional().describe("Attributes for trace or event")
  }),
    ...
    try {
      switch (args.action) {
        case 'start':
          ...
          const spanId = await analyticsTools.startTrace(args.traceName, args.attributes || {});
          return {
            success: true,
            output: JSON.stringify({
              spanId,
        ...
        case 'event':
          ...
          analyticsTools.analytics.addTraceEvent(args.spanId, args.eventName, args.attributes || {});
          return {
            success: true,
            output: JSON.stringify({
              spanId: args.spanId,
...
export const vercelTracingTool: Tool = {
  name: "vercel_ai_tracing",
  description: "Vercel AI SDK tracing integration for detailed request tracking, performance monitoring, and cost analysis.",
  parameters: z.object({
    action: z.enum(["record", "stats", "export"]).describe("Tracing action to perform"),
    trace: z.object({
    ...
    }).optional().describe("Trace data to record"),
    timeRange: z.number().optional().describe("Time range for stats in milliseconds"),
    format: z.enum(["json", "csv"]).optional().describe("Export format")
  }),
  ...
  async execute(args: any): Promise<ToolResult> {
    ...
    try {
      switch (args.action) {
        ...
        case 'stats':
          const stats = (analytics as any).getVercelTraceStats(args.timeRange || 24 * 60 * 60 * 1000);
          return {
            success: true,
            output: JSON.stringify({
        ...
        case 'export':
          const exportData = await analytics.exportData(args.format || 'json', true);
          return {
            success: true,
            output: args.format === 'csv' ? exportData : JSON.stringify({
...
export const debugConsoleTool: Tool = {
  name: "debug_console",
  description: "Visual debug console for real-time monitoring of Krab system activity, errors, and performance metrics.",
  parameters: z.object({
    action: z.enum(["view", "add_entry", "clear", "generate_html"]).describe("Debug console action"),
    level: z.enum(["info", "warn", "error", "debug"]).optional().describe("Log level for entries"),
    category: z.enum(["request", "response", "error", "system", "agent", "tool"]).optional().describe("Log category"),
  ...
  async execute(args: any): Promise<ToolResult> {
    ...
    try {
      switch (args.action) {
        case 'view':
          const entries = (analytics as any).getDebugEntries(args.limit || 50, args.level, args.category);
          return {
            success: true,
            output: JSON.stringify({
...
export const learningSystemTool: Tool = {
  name: "learning_system",
  description: "AI-powered learning system that analyzes system behavior, identifies patterns, and provides actionable insights for optimization.",
  parameters: z.object({
    action: z.enum(["generate_insights", "apply_insight", "get_insights", "analyze_errors"]).describe("Learning system action"),
    ...
    try {
      switch (args.action) {
        ...
        case 'apply_insight':
          ...
          const applied = (analytics as any).applyLearningInsight(args.insightId);
          return {
            success: applied,
            output: applied ? `Insight ${args.insightId} applied successfully` : `Insight ${args.insightId} not found`
          };

        case 'get_insights':
          const allInsights = (analytics as any).getLearningInsights(args.limit || 20);
          return {
            success: true,
            output: JSON.stringify({
