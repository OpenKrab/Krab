...
- **TUI/CLI First**: เน้นการใช้งานผ่าน Terminal ที่ทรงพลังและสวยงาม (ไม่มี Dashboard กวนใจ).

## ⚖️ Non-Dashboard Parity Roadmap

เป้าหมายของ parity pass นี้คือทำให้ Krab เข้าใกล้ OpenClaw ในเส้นทาง operator/core runtime โดยไม่ขยาย scope ไปสู่ dashboard, mobile apps, web control UI, หรือ extension ecosystem เต็มรูปแบบ

### โฟกัสที่ทำจริงในรอบนี้

- **Provider/runtime polish**
  - deterministic provider detection
  - explicit override และ diagnostics
  - fallback visibility ใน CLI

- **Gateway/runtime cohesion**
  - shared runtime snapshot ระหว่าง gateway/CLI
  - session + presence + queue summary ที่ใช้ shape เดียวกัน
  - readiness/runtime checks สำหรับ operator commands
...
- **Docs/release parity**
  ...
  - changelog
  - parity report
  - command documentation ที่ตรงกับ implementation จริง

### ยังไม่รวมใน parity รอบนี้

- dashboard/TUI parity เพิ่มเติม
- native/macOS/iOS/Android app parity
- web control UI parity
- extension/plugin ecosystem parity แบบ OpenClaw monorepo
...
- [x] สร้าง **Terminal UI (TUI)** ที่สวยงามและตอบโจทย์การใช้งานจริง
- [ ] สร้าง Web UI (Control Panel) — [PAUSED: Focus on TUI/CLI]
- [x] Docker deployment
...
- [x] 🏠 **Obsidian Integration** (Advanced Knowledge Base)
  - [x] Basic vault reading, note parsing, and writing
  - [x] **TF-IDF Semantic Search** - Cosine similarity-based content search
  - [x] **Knowledge Graph Analysis** - Note relationships, centrality scoring, path finding
  - [x] **AI Knowledge Synthesis** - Automated topic summarization and concept clustering
  - [x] **Real-time Sync** - File watching with automatic re-indexing
  - [x] **Advanced CLI Commands** - 15+ sophisticated Obsidian tools
  - [x] Integration with Krab's tool registry and CLI system
- [x] 🧩 **Plugin Ecosystem** (Dynamic plugin loader, scaffolding, manifest system)
- [ ] 📱 **Mobile App** (React Native/Expo) - [Future]
- [ ] 🌐 **Web UI** (Full dashboard) - [ON HOLD]
